...
std::map< BlkId, IndexBufferPtr > IndexCPContext::recover(sisl::byte_view sb) {
    ...
    // Lambda function to modify the DAG
    auto modifyBuffer = [](IndexBufferPtr& buffer) {
        ...
        if (real_up_buf != up_buf) {
            ...
            LOGTRACEMOD(wbcache,"change upbuffer from {} to {}", up_buf->to_string(), buffer->m_up_buffer->to_string());
        }
    };
    auto dag_print = [](const std::map< BlkId, IndexBufferPtr >& dags, std::string delimiter) {
        int index = 1;
        for (const auto& [blkid, bufferPtr] : dags) {
            LOGTRACEMOD(wbcache,"{}{} - blkid {} buffer {} ",delimiter, index++, blkid.to_integer(),bufferPtr->to_string());
        }
    };
    LOGTRACEMOD(wbcache,"\n\n\nbefore modify : \n ");
    dag_print(buf_map, "bef ");
    // Apply the lambda function to each buffer in the map
    for (auto& [blkid, bufferPtr] : buf_map) {
        modifyBuffer(bufferPtr);
    }
    LOGTRACEMOD(wbcache,"\n\n\nAFTER modify : \n ");
    dag_print(buf_map, "aft ");

//    auto sanityCheck = [](const std::map< BlkId, IndexBufferPtr >& dags) {
//        for (const auto& [blkid, bufferPtr] : dags) {
//            auto up_buffer = bufferPtr->m_up_buffer;
//            if (up_buffer) {
////                HS_REL_ASSERT(!up_buffer->m_node_freed,
////                              "Sanity check failed: Buffer {} blkdid {} has an up_buffer {} blkid that is marked as freed.",
////                              bufferPtr->to_string(), blkid.to_integer(), up_buffer->to_string(), up_buffer->blkid().to_integer());
...
//                              "Sanity check failed: up_buffer {} has't {} as a down_buffer.",
//                              up_buffer->to_string(), bufferPtr->to_string());
//#endif
//            }
//
    ...
    auto const rec_to_buf = [&buf_map, &cpg](txn_record const* rec, bool is_meta, BlkId const& bid,
        ...
        if (up_buf) {
            ...
            if (up_buf->m_created_cp_id == cpg->id()) {
                ...
                real_up_buf = up_buf->m_up_buffer;
                LOGTRACEMOD(wbcache,"\n\n change upbuffer from {} to {}\n\n", up_buf->to_string(), real_up_buf->to_string());
            }
...
std::string IndexCPContext::txn_record::to_string() const {
    ...
    if(num_freed_ids) {fmt::format_to(std::back_inserter(str), ", freed_node_level= {}", (uint8_t) (free_node_level));};
