...
std::map< BlkId, IndexBufferPtr > IndexCPContext::recover(sisl::byte_view sb) {
    ...
    //            LOGTRACEMOD(wbcache,"{}{} - blkid {} buffer {} ",delimiter, index++,
    //            blkid.to_integer(),bufferPtr->to_string());
    //        }
    //    };
    //    LOGTRACEMOD(wbcache,"\n\n\nbefore modify : \n ");
    ...
    auto sanityCheck = [](const std::map< BlkId, IndexBufferPtr >& dags) {
        for (const auto& [blkid, bufferPtr] : dags) {
            ...
            if (up_buffer) {
                ...
                //                              "Sanity check failed: Buffer {} blkdid {} has an up_buffer {} blkid that
                //                              is marked as freed.", bufferPtr->to_string(), blkid.to_integer(),
                //                              up_buffer->to_string(), up_buffer->blkid().to_integer());
