...
static constexpr uint32_t g_num_reactors{4};
...
public:
    ...
    void SetUp() override {
        ...
        create_io_reactors(g_num_reactors);
    ...
    void create_io_reactors(uint32_t num_io_reactors) {
        ...
        LOGINFO("Created {} IO reactors", m_reactors.size());
    }
    ...
    std::latch m_test_done_latch{g_num_reactors};
...
TEST_F(TestIndexGC, chunk_gc_test) {
    ...
    iomanager.run_on_forget(m_reactors[0], [this]() { gc_task(0); });
    iomanager.run_on_forget(m_reactors[1], [this]() { gc_task(1); });
    iomanager.run_on_forget(m_reactors[2], [this]() { put_task(2); });
    iomanager.run_on_forget(m_reactors[3], [this]() { get_task(3); });
